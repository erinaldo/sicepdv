    ...
    class FuncoesNFC
    {
        ...
        public string urlConsultaNFC(string estado, string hambiente)
        {
            ...
            try
            {
                ...
                {
                    LogSICEpdv.Registrarlog("Process myProcess = System.Diagnostics.Process.Start(@'C:/iqsistemas/SICENFC-e/NFCePrint.exe', ' ' + documentos.chaveNFC + '' + ConfiguracoesECF.NFCModeloImpressao + '' + ConfiguracoesECF.NFCNomeImpressora + '' + string.Format('{0:N2}', documentos.troco))","", "FuncoesNFC.cs");
                    //MessageBox.Show("|" + documentos.chaveNFC + "|" + ConfiguracoesECF.NFCModeloImpressao + "|" + ConfiguracoesECF.NFCNomeImpressora + "|" + string.Format("{0:N2}", documentos.troco));
        ...
        static public void verificarGerenciadorNFCe(string acao = "Abrir")
        {
            try
            {
                //if (ConfiguracoesECF.NFCtransacaobanco == false)
        ...
        static public List<CaixaFiscal> valoresCaixaNFCe(DateTime dataIncio, DateTime dataFinal)
        {
            string SQL = "SELECT i.CbdCST AS tributacao, i.CbdpICMS AS icms, TRUNCATE(SUM(v.CbdvProd), 2) - TRUNCATE(SUM(v.CbdvDesc), 2) AS baseICMS, TRUNCATE(SUM(i.CbdvICMS_icms), 2) AS imposto FROM cbd001 AS c, cbd001deticmsnormalst AS i, cbd001det AS v " +
                                                                            "WHERE DATE(c.cbddVenda) BETWEEN '" + dataIncio.ToString("yyyy-MM-dd") + "' AND '" + dataFinal.ToString("yyyy-MM-dd") + "'  " +
        ...
        static public List<CaixaFiscal> valoresCaixaCanceladoNFCe(DateTime dataIncio, DateTime dataFinal)
        {
            string SQL = "SELECT i.CbdCST AS tributacao, i.CbdpICMS AS icms, TRUNCATE(SUM(v.CbdvProd), 2) - TRUNCATE(SUM(v.CbdvDesc), 2) AS baseICMS, TRUNCATE(SUM(i.CbdvICMS_icms), 2) AS imposto FROM cbd001 AS c, cbd001deticmsnormalst AS i, cbd001det AS v " +
                                                                            "WHERE DATE(c.cbddVenda) BETWEEN '" + dataIncio.ToString("yyyy-MM-dd") + "' AND '" + dataFinal.ToString("yyyy-MM-dd") + "'  " +
                                                                            "AND i.CbdNtfNumero = c.CbdNtfNumero AND i.CbdNtfSerie = c.CbdNtfSerie " +
