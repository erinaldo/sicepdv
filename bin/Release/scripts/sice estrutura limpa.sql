...
CREATE TABLE `auditoria` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=65664 DEFAULT CHARSET=latin1;
...
CREATE TABLE `caixas` (
  ...
  `ecfnumero` varchar(3) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=1965 DEFAULT CHARSET=latin1;
...
CREATE TABLE `consultaclientes` (
  ...
  KEY `codigo` (`codigo`),
  KEY `cpfcnpj` (`cpfcnpj`)
) ENGINE=MyISAM AUTO_INCREMENT=36100 DEFAULT CHARSET=latin1;
...
CREATE TABLE `contdocs` (
  ...
  KEY `ip` (`ip`),
  KEY `codigocliente` (`codigocliente`)
) ENGINE=MyISAM AUTO_INCREMENT=85396 DEFAULT CHARSET=latin1;
...
CREATE TABLE `contnfsaida` (
  ...
  `notafiscalservico` char(1) NOT NULL DEFAULT 'N',
  `nrefcuf` char(2) DEFAULT NULL,
  `nrefaamm` char(4) DEFAULT NULL,
  `nrefcnpj` varchar(18) DEFAULT NULL,
  `nrefmodelo` char(5) DEFAULT NULL,
  `nrefserie` char(3) DEFAULT NULL,
  `nrefnnf` int(11) DEFAULT NULL,
  `nrefnECF` int(3) DEFAULT NULL,
  `nrefnCOO` decimal(6,0) DEFAULT NULL,
  `nrefCPF` varchar(11) DEFAULT NULL,
  `nrefmodECF` char(2) DEFAULT NULL,
  `nrefnfe` char(44) DEFAULT NULL,
  PRIMARY KEY (`inc`)
) ENGINE=MyISAM AUTO_INCREMENT=5 DEFAULT CHARSET=latin1;
...
) ENGINE=MyISAM AUTO_INCREMENT=78250 DEFAULT CHARSET=latin1;
...
CREATE TABLE `crmovclientespagas` (
  ...
  KEY `IndNomecodigo` (`nome`,`codigo`),
  KEY `datacompra` (`datacompra`)
) ENGINE=MyISAM AUTO_INCREMENT=780804 DEFAULT CHARSET=latin1;
...
CREATE TABLE `entradas` (
  ...
  `icmsst` decimal(8,2) NOT NULL DEFAULT '0.00',
  PRIMARY KEY (`inc`),
  KEY `numero` (`numero`)
...
CREATE TABLE `filiais` (
  ...
  `usuarioemail` varchar(50) DEFAULT NULL,
...
) ENGINE=MyISAM AUTO_INCREMENT=113955 DEFAULT CHARSET=latin1;
...
CREATE TABLE `produtoscomposicao` (
  ...
  `custo` decimal(10,5) NOT NULL DEFAULT '0.00000',
  `custototal` decimal(10,5) NOT NULL DEFAULT '0.00000',
  PRIMARY KEY (`inc`)
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `AjustarCamposNulosManual`()
BEGIN
...
UPDATE entradas SET icmsentrada=(valoricms/totalitem)*100  WHERE tributacao="000" AND icmsentrada=0 AND bcicms>0 AND valoricms>0; 
...
update moventradas set exportarfiscal="N" where cfopentrada="1.906";
UPDATE contnfsaida SET exportarfiscal="N" WHERE cfop="5.905";
UPDATE contnfsaida SET exportarfiscal="N" WHERE cfop="1.152";
if ( (select filiais.tipoempresa from filiais limit 1)="1") then
UPDATE produtos SET pis=0.65 WHERE tributacaoPIS="01" AND pis=0;
UPDATE produtosfilial SET cofins=3 WHERE tributacaoCOFINS="01" AND cofins=0;
UPDATE entradas SET pis=0.65 WHERE cstpis="01";
UPDATE entradas SET cofins=3 WHERE cstcofins="01" ;
UPDATE entradas SET cstpis="50" WHERE cstpis="01";
UPDATE entradas SET cstcofins="50" WHERE cstcofins="01";
UPDATE vendaarquivo SET pis=0.65 WHERE cstpis="01";
UPDATE vendaarquivo SET cofins=3 WHERE cstcofins="01" ;
UPDATE vendanf SET pis=0.65 WHERE cstpis="01" ;
UPDATE vendanf SET cofins=3 WHERE cstcofins="01";   
END IF;
IF ( (SELECT filiais.tipoempresa FROM filiais LIMIT 1)="3") THEN
UPDATE produtos SET pis=1.65 WHERE tributacaoPIS="01";
UPDATE produtosfilial SET cofins=7.6 WHERE tributacaoCOFINS="01" ;
UPDATE entradas SET pis=1.65 WHERE cstpis="01" ;
UPDATE entradas SET cofins=7.6 WHERE cstcofins="01";
UPDATE entradas SET cstpis="50" WHERE cstpis="01";
UPDATE entradas SET cstcofins="50" WHERE cstcofins="01";
UPDATE vendaarquivo SET pis=1.65 WHERE cstpis="01" ;
UPDATE vendaarquivo SET cofins=7.6 WHERE cstcofins="01" ;
UPDATE vendanf SET pis=1.65 WHERE cstpis="01" ;
UPDATE vendanf SET cofins=7.6 WHERE cstcofins="01";   
END IF;
UPDATE entradas SET tributacao="000" WHERE tributacao="070" AND entradas.percentualRedBaseCalcICMS=0;
UPDATE entradas SET tributacao="000" WHERE tributacao="020" AND entradas.percentualRedBaseCalcICMS=0;
UPDATE entradas SET tributacao="000" WHERE tributacao="010" AND entradas.percentualRedBaseCalcICMS=0;
...
update produtos set descontomaximo=0.01 where descontomaximo=0; 
UPDATE produtosfilial SET descontomaximo=0.01 WHERE descontomaximo=0;
...
if ( (select abaterestoqueprevenda from configfinanc where codigofilial=filial limit 1) = "S") THEN	
	...
	AND produtos.codigofilial=filial
	AND vendadav.codigofilial = produtos.codigo
	...
	AND produtosfilial.codigofilial=filial
	AND vendadav.codigofilial = produtosfilial.codigo
	AND TO_DAYS(CURRENT_DATE)-TO_DAYS(vendadav.DATA)>30;
...
if ( (select diasparamudarsituacao from configfinanc where codigofilial=filial limit 1)>0) THEN	
  IF ( (SELECT diasparamudarsituacaoinferior FROM configfinanc WHERE codigofilial=filial)>0) THEN	
...
WHERE vendas.id=ipTerminal;
 ...
 set ',@tabelaProduto,'.qtdanterior=',@tabelaProduto,'.quantidade ,'
 ,@tabelaProduto,'.quantidade=',@tabelaProduto,'.quantidade-(SELECT IFNULL(SUM(vendas.quantidade*vendas.embalagem),0) from vendas where vendas.cancelado="N" AND vendas.codigo=',@tabelaProduto,'.codigo and vendas.id=','"',ipTerminal,'"),',
...
END IF;
 ...
 AND 	produtosgrade.grade=devolucao.grade 
 AND 	produtosgrade.codigofilial=filial;
 DELETE FROM caixa WHERE tipopagamento="DV" and enderecoip=ipTerminal AND documento IS NULL;
    END */$$
DELIMITER ;
...
/*!50001 CREATE TABLE  `blococregc190`(
 ...
 `totalPIS` decimal(39,2) ,
 `totalCOFINS` decimal(39,2) ,
...
/*!50001 CREATE TABLE  `blococregc190_saida`(
 ...
 `nrcontrole` int(5) ,
 `totalicms` decimal(47,2) ,
 `totalIPI` decimal(47,2) ,
 `totalPIS` decimal(40,2) ,
 `totalCOFINS` decimal(40,2) ,
 `descontovalor` decimal(32,2) ,
 `SUM(TOTAL)` decimal(32,2) ,
 `baseCalculoICMS` decimal(37,2) 
)*/;
...
/*!50001 CREATE TABLE  `registro50saidas_itens`(
 ...
 `baseCalculoPIS` decimal(37,2) ,
 `baseCalculoCOFINS` decimal(37,2) 
)*/;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `blococregc190` AS select `entradas`.`codigofilial` AS `codigofilial`,`entradas`.`numero` AS `numero`,`entradas`.`NF` AS `nf`,`entradas`.`modeloNF` AS `modelonf`,`entradas`.`dataentrada` AS `dataentrada`,`entradas`.`cfopentrada` AS `cfopentrada`,`entradas`.`IcmsEntrada` AS `icmsentrada`,`entradas`.`tributacao` AS `tributacao`,`entradas`.`codigo` AS `codigo`,`entradas`.`descricao` AS `descricao`,`entradas`.`sequencia` AS `sequencia`,sum(`entradas`.`quantidade`) AS `quantidade`,`entradas`.`unidade` AS `unidade`,`entradas`.`Custo` AS `custo`,`entradas`.`percentualRedBaseCalcICMS` AS `percentualRedBaseCalcICMS`,sum(`entradas`.`bcicms`) AS `bcicms`,round(sum((`entradas`.`bcicms` * (`entradas`.`IcmsEntrada` / 100))),2) AS `toticms`,sum(round((`entradas`.`totalitem` * (`entradas`.`IPI` / 100)),2)) AS `ipiItem`,sum(round((`entradas`.`totalitem` * (`entradas`.`pis` / 100)),2)) AS `totalPIS`,sum(round((`entradas`.`totalitem` * (`entradas`.`cofins` / 100)),2)) AS `totalCOFINS`,sum(`entradas`.`totalitem`) AS `totalProduto`,(sum(round((`entradas`.`totalitem` * (`entradas`.`IPI` / 100)),2)) + sum(`entradas`.`totalitem`)) AS `totalNF`,sum(round((`entradas`.`totalitem` * (`entradas`.`percentualRedBaseCalcICMS` / 100)),2)) AS `totalReducaoICMS` from `entradas` group by `entradas`.`NF`,`entradas`.`cfopentrada`,`entradas`.`IcmsEntrada`,`entradas`.`tributacao`,`entradas`.`numero` order by `entradas`.`NF`,`entradas`.`sequencia` */;

/*View structure for view blococregc190_saida */

/*!50001 DROP TABLE IF EXISTS `blococregc190_saida` */;
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `blococregc190_saida` AS select `vendanf`.`codigofilial` AS `codigofilial`,`vendanf`.`NotaFiscal` AS `notafiscal`,`vendanf`.`serieNF` AS `serienf`,`vendanf`.`modelodocfiscal` AS `modelodocfiscal`,`vendanf`.`documento` AS `documento`,`vendanf`.`data` AS `DATA`,`vendanf`.`cfop` AS `cfop`,`vendanf`.`icms` AS `icms`,`vendanf`.`tributacao` AS `tributacao`,`vendanf`.`codigo` AS `codigo`,`vendanf`.`produto` AS `produto`,sum(`vendanf`.`quantidade`) AS `SUM(quantidade)`,`vendanf`.`unidade` AS `unidade`,`vendanf`.`nrcontrole` AS `nrcontrole`,sum(round((((`vendanf`.`total` - ((`vendanf`.`total` * `vendanf`.`percentualRedBaseCalcICMS`) / 100)) * `vendanf`.`icms`) / 100),2)) AS `totalicms`,sum(round((((`vendanf`.`total` - ((`vendanf`.`total` * `vendanf`.`percentualRedBaseCalcICMS`) / 100)) * `vendanf`.`ipi`) / 100),2)) AS `totalIPI`,sum(round((((`vendanf`.`total` - ((`vendanf`.`total` * `vendanf`.`percentualRedBaseCalcICMS`) / 100)) * `vendanf`.`pis`) / 100),2)) AS `totalPIS`,sum(round((((`vendanf`.`total` - ((`vendanf`.`total` * `vendanf`.`percentualRedBaseCalcICMS`) / 100)) * `vendanf`.`cofins`) / 100),2)) AS `totalCOFINS`,sum(`vendanf`.`descontovalor`) AS `descontovalor`,sum(`vendanf`.`total`) AS `SUM(TOTAL)`,if((`vendanf`.`icms` > 0),sum(round((`vendanf`.`total` - (`vendanf`.`total` * (`vendanf`.`percentualRedBaseCalcICMS` / 100))),2)),0) AS `baseCalculoICMS` from `vendanf` group by `vendanf`.`NotaFiscal`,`vendanf`.`serieNF`,`vendanf`.`icms`,`vendanf`.`cfop`,`vendanf`.`codigofilial`,`vendanf`.`tributacao` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `blococregc481_pis` AS select `contdocs`.`documento` AS `documento`,`contdocs`.`data` AS `data`,`contdocs`.`ecffabricacao` AS `ecffabricacao`,`contdocs`.`ecfnumero` AS `ecfnumero`,`contdocs`.`ncupomfiscal` AS `ncupomfiscal`,`contdocs`.`modeloDOCFiscal` AS `modelodocfiscal`,`vendatmp`.`codigo` AS `codigo`,`vendatmp`.`produto` AS `descricao`,sum(truncate(`vendatmp`.`quantidade`,3)) AS `quantidade`,`vendatmp`.`unidade` AS `unidade`,`vendatmp`.`preco` AS `preco`,`vendatmp`.`descontovalor` AS `descontovalor`,`vendatmp`.`Descontoperc` AS `descontoperc`,sum(truncate(((`vendatmp`.`total` + `vendatmp`.`rateioencargos`) - `vendatmp`.`ratdesc`),2)) AS `total`,truncate(((sum(((`vendatmp`.`total` + `vendatmp`.`rateioencargos`) - `vendatmp`.`ratdesc`)) * `vendatmp`.`pis`) / 100),2) AS `totalPIS`,truncate(((sum(((`vendatmp`.`total` + `vendatmp`.`rateioencargos`) - `vendatmp`.`ratdesc`)) * `vendatmp`.`cofins`) / 100),2) AS `totalCOFINS`,`vendatmp`.`icms` AS `icms`,`vendatmp`.`pis` AS `pis`,`vendatmp`.`cofins` AS `cofins`,`vendatmp`.`cstpis` AS `cstpis`,`vendatmp`.`cstcofins` AS `cstcofins`,`vendatmp`.`tributacao` AS `tributacao`,`vendatmp`.`cfop` AS `cfop` from (`contdocs` join `vendatmp`) where ((`contdocs`.`documento` = `vendatmp`.`documento`) and (`contdocs`.`estornado` = 'N') and (`vendatmp`.`quantidade` > 0) and (`vendatmp`.`cancelado` = 'N') and (`vendatmp`.`modelodocfiscal` = '2D')) group by `vendatmp`.`codigo` order by `vendatmp`.`nrcontrole` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `registro50entradas_itens` AS select `entradas`.`codigofilial` AS `codigofilial`,`entradas`.`numero` AS `numero`,`entradas`.`NF` AS `nf`,`entradas`.`modeloNF` AS `modelonf`,`entradas`.`dataentrada` AS `dataentrada`,`entradas`.`cfopentrada` AS `cfopentrada`,`entradas`.`IcmsEntrada` AS `icmsentrada`,`entradas`.`IPI` AS `ipi`,`entradas`.`pis` AS `pis`,`entradas`.`cofins` AS `cofins`,`entradas`.`cstpis` AS `cstpis`,`entradas`.`cstcofins` AS `cstcofins`,`entradas`.`tributacao` AS `tributacao`,`entradas`.`codigo` AS `codigo`,`entradas`.`descricao` AS `descricao`,`entradas`.`sequencia` AS `sequencia`,sum(`entradas`.`quantidade`) AS `quantidade`,`entradas`.`unidade` AS `unidade`,`entradas`.`Custo` AS `custo`,sum(`entradas`.`bcicms`) AS `bcicms`,round(sum((`entradas`.`bcicms` * (`entradas`.`IcmsEntrada` / 100))),2) AS `toticms`,if((`entradas`.`IPI` > 0),sum(round((`entradas`.`totalitem` * (`entradas`.`IPI` / 100)),2)),0) AS `ipiItem`,if((`entradas`.`pis` > 0),round(sum((`entradas`.`bcicms` * (`entradas`.`pis` / 100))),2),0) AS `totPIS`,if((`entradas`.`cofins` > 0),round(sum((`entradas`.`bcicms` * (`entradas`.`cofins` / 100))),2),0) AS `totCOFINS`,sum(`entradas`.`totalitem`) AS `totalProduto`,(sum(round((`entradas`.`totalitem` * (`entradas`.`IPI` / 100)),2)) + sum(`entradas`.`totalitem`)) AS `totalNF`,if((`entradas`.`IPI` > 0),sum(`entradas`.`bcicms`),0) AS `baseCalculoIPI`,if((`entradas`.`pis` > 0),sum(`entradas`.`bcicms`),0) AS `baseCalculoPIS`,if((`entradas`.`cofins` > 0),sum(`entradas`.`bcicms`),0) AS `baseCalculoCOFINS` from `entradas` group by `entradas`.`NF`,`entradas`.`inc` order by `entradas`.`NF`,`entradas`.`sequencia` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `registro50saidas_itens` AS select `vendanf`.`inc` AS `inc`,`vendanf`.`codigofilial` AS `codigofilial`,`vendanf`.`NotaFiscal` AS `notafiscal`,`vendanf`.`serieNF` AS `serienf`,`vendanf`.`modelodocfiscal` AS `modelodocfiscal`,`vendanf`.`documento` AS `documento`,`vendanf`.`data` AS `DATA`,`vendanf`.`cfop` AS `cfop`,`vendanf`.`icms` AS `icms`,`vendanf`.`ipi` AS `ipi`,`vendanf`.`pis` AS `pis`,`vendanf`.`cofins` AS `cofins`,`vendanf`.`cstpis` AS `cstpis`,`vendanf`.`cstcofins` AS `cstcofins`,`vendanf`.`tributacao` AS `tributacao`,`vendanf`.`codigo` AS `codigo`,`vendanf`.`produto` AS `produto`,sum(`vendanf`.`quantidade`) AS `SUM(quantidade)`,`vendanf`.`unidade` AS `unidade`,`vendanf`.`nrcontrole` AS `nrcontrole`,sum(round((((`vendanf`.`total` - ((`vendanf`.`total` * `vendanf`.`percentualRedBaseCalcICMS`) / 100)) * `vendanf`.`icms`) / 100),2)) AS `totalicms`,if((`vendanf`.`ipi` > 0),sum(round((((`vendanf`.`total` - ((`vendanf`.`total` * `vendanf`.`percentualRedBaseCalcICMS`) / 100)) * `vendanf`.`ipi`) / 100),2)),0) AS `totalIPI`,if((`vendanf`.`pis` > 0),sum(round((((`vendanf`.`total` - ((`vendanf`.`total` * `vendanf`.`percentualRedBaseCalcICMS`) / 100)) * `vendanf`.`pis`) / 100),2)),0) AS `totalPIS`,if((`vendanf`.`cofins` > 0),sum(round((((`vendanf`.`total` - ((`vendanf`.`total` * `vendanf`.`percentualRedBaseCalcICMS`) / 100)) * `vendanf`.`cofins`) / 100),2)),0) AS `totalCOFINS`,sum(`vendanf`.`descontovalor`) AS `descontovalor`,sum(`vendanf`.`total`) AS `SUM(TOTAL)`,if((`vendanf`.`icms` > 0),sum(round((`vendanf`.`total` - (`vendanf`.`total` * (`vendanf`.`percentualRedBaseCalcICMS` / 100))),2)),0) AS `baseCalculoICMS`,if((`vendanf`.`ipi` > 0),sum(round((`vendanf`.`total` - (`vendanf`.`total` * (`vendanf`.`percentualRedBaseCalcICMS` / 100))),2)),0) AS `baseCalculoIPI`,if((`vendanf`.`pis` > 0),sum(round((`vendanf`.`total` - (`vendanf`.`total` * (`vendanf`.`percentualRedBaseCalcICMS` / 100))),2)),0) AS `baseCalculoPIS`,if((`vendanf`.`cofins` > 0),sum(round((`vendanf`.`total` - (`vendanf`.`total` * (`vendanf`.`percentualRedBaseCalcICMS` / 100))),2)),0) AS `baseCalculoCOFINS` from `vendanf` group by `vendanf`.`inc` order by `vendanf`.`NotaFiscal`,`vendanf`.`nrcontrole` */;
